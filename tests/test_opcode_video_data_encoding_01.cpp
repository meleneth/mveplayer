...
TEST_CASE("OpcodeVideoData#process_encoding_01 does nothing")
{
  using namespace mve;
  ...
  std::copy(nv.begin(), nv.end(), movie_player.new_frame->raw_data.begin());

  opcode.process_encoding_01(0, 0, movie_player);

  REQUIRE_THAT(movie_player.new_frame->raw_data, BytesEqualWindow(nv));
